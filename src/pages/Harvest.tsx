...
const ActivityLog: React.FC<{ maxH?: number }> = ({ maxH = 280 }) => {
  ...
  const colors: Record<string, string> = { ENTRY: 'var(--accent-green)', ENTER_PERP: 'var(--accent-green)', ENTER_SPOT: 'var(--accent-green)', ENTER_COMPLETE: 'var(--accent-green)', ENTER_FAILED: 'var(--accent-red)', EXIT: 'var(--accent-yellow)', EXIT_PERP: 'var(--accent-yellow)', EXIT_SPOT: 'var(--accent-yellow)', EXIT_COMPLETE: 'var(--accent-yellow)', EXIT_FAILED: 'var(--accent-red)', ROTATE: 'var(--hl-teal)', REBALANCE: 'var(--hl-teal)', FUNDING_ACCRUED: 'var(--accent-green)', REGIME_CHANGE: 'var(--hl-teal)', SKIP: 'var(--text-muted)', ERROR: 'var(--accent-red)', INFO: 'var(--text-secondary)' };
  const icons: Record<string, string> = { ENTRY: '↗', ENTER_PERP: '↗', ENTER_SPOT: '↗', ENTER_COMPLETE: '✓', ENTER_FAILED: '✗', EXIT: '↙', EXIT_PERP: '↙', EXIT_SPOT: '↙', EXIT_COMPLETE: '✓', EXIT_FAILED: '✗', ROTATE: '↻', REBALANCE: '⇄', FUNDING_ACCRUED: '$', REGIME_CHANGE: '◈', SKIP: '–', ERROR: '⚠', INFO: '·' };
...
const ConfigPanel: React.FC<{
  config: ReturnType<typeof useHarvestStore.getState>['config'];
  update: (d: Partial<ReturnType<typeof useHarvestStore.getState>['config']>) => void;
}> = ({ config, update }) => (
  <>
    <ConfigRow
  ...
  if (isMobile) {
    return (
      <div className="fade-in" style={{ paddingTop: 'var(--sp-3)', paddingBottom: 80 }}>
        ...
        <Accordion
          title="Activity Log"
          subtitle={autoPositions.length > 0 || useHarvestStore.getState().log.length > 0 ? `${useHarvestStore.getState().log.length} events` : 'No events yet'}
          defaultOpen={false}
        >
