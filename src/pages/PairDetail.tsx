...
import { computeBreakEvenHours } from '../api/fees';
    ...
    <div className="fade-in" style={{ paddingTop: 'var(--sp-4)', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 'var(--sp-3)', marginBottom: 'var(--sp-4)' }}>
        ...
        <div className="glass-card" style={{ padding: 'var(--sp-4)' }}>
          <p style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>7d Rate Percentile</p>
          {pctile !== null ? (
            <>
              <p style={{
                fontFamily: 'var(--font-mono)', fontSize: 18, fontWeight: 700,
                color: pctile >= 75 ? 'var(--accent-green)' : pctile >= 40 ? 'var(--accent-yellow)' : 'var(--text-muted)',
              }}>{pctile}th</p>
              <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                {pctile >= 75 ? 'High vs recent history' : pctile >= 40 ? 'Average vs recent history' : 'Low vs recent history'}
              </p>
            </>
          ) : (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-muted)' }}>—</p>
          )}
        </div>

        {/* Persistence hours */}
        <div className="glass-card" style={{ padding: 'var(--sp-4)' }}>
          <p style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>Consecutive Hours Above Threshold</p>
          <p style={{
            fontFamily: 'var(--font-mono)', fontSize: 18, fontWeight: 700,
            color: (pair?.persistenceHours ?? 0) >= 24 ? 'var(--accent-green)' : (pair?.persistenceHours ?? 0) >= 6 ? 'var(--accent-yellow)' : 'var(--text-muted)',
          }}>{pair?.persistenceHours ?? 0}h</p>
          <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>persistence</p>
        </div>

        {/* Avg hold time */}
        <div className="glass-card" style={{ padding: 'var(--sp-4)' }}>
          <p style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>Avg Hold (Your Trades)</p>
          {avgHoldHours !== null ? (
            <>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 18, fontWeight: 700 }}>{avgHoldHours.toFixed(0)}h</p>
              <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{pairTrades.length} trade{pairTrades.length !== 1 ? 's' : ''}</p>
            </>
          ) : (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--text-muted)' }}>No history yet</p>
          )}
        </div>

        {/* Break-even */}
        <div className="glass-card" style={{ padding: 'var(--sp-4)' }}>
          <p style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>Break-even Hours</p>
          {breakEvenHrs !== null ? (
            <>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 18, fontWeight: 700, color: breakEvenHrs <= 24 ? 'var(--accent-green)' : 'var(--accent-yellow)' }}>
                {breakEvenHrs.toFixed(1)}h
              </p>
              <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>to cover round-trip fees</p>
            </>
          ) : (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-muted)' }}>—</p>
          )}
        ...
        <div className="glass-card" style={{ padding: 'var(--sp-4)' }}>
          ...
          {loading ? (
            ...
            <div style={{ height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 6 }}>
              <span style={{ color: 'var(--accent-red)', fontSize: 12, fontWeight: 600 }}>⚠ API Error</span>
              <span style={{ color: 'var(--text-muted)', fontSize: 11, textAlign: 'center', maxWidth: 220 }}>{dataError}</span>
            </div>
          ) : (
      ...
      <div style={{ marginTop: 'var(--sp-6)', paddingTop: 'var(--sp-4)', borderTop: '1px solid var(--glass-border)' }}>
        {showEntry && pair && <EntryModal pair={pair} onClose={() => setShowEntry(false)} />}
        <button
          className="btn btn-primary"
          style={{ width: '100%', padding: 'var(--sp-3)', fontSize: 14, fontWeight: 700 }}
