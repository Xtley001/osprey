...
const BT_DEFAULTS: StrategyParams = {
  ...
  takerFee:           0.00045,   // overridden by live fee fetch — see useEffect below
  makerFee:           0.00015,   // overridden by live fee fetch — see useEffect below
};
...
const Backtester: React.FC = () => {
  ...
  // Sync live fees from feeStore into strategy when they update
  const liveFees = useFeeStore(s => s.fees);
  React.useEffect(() => {
    setStrategy(s => ({
      ...s,
      takerFee: liveFees.perpTaker,
      makerFee: liveFees.perpMaker,
    }));
  }, [liveFees.perpTaker, liveFees.perpMaker]);
