...
export const EntryModal: React.FC<EntryModalProps> = ({ pair, onClose }) => {
  ...
  // Entry: both perp and spot legs at maker fee (Alo post-only order)
  // notional here is the TOTAL capital; each leg is notional/2
  const perpEntryFee   = (notional / 2) * fees.perpMaker;
  const spotEntryFee   = (notional / 2) * fees.spotMaker;
  const entryFee       = perpEntryFee + spotEntryFee;
  // Exit: both legs at taker fee
  const perpExitFee    = (notional / 2) * fees.perpTaker;
  const spotExitFee    = (notional / 2) * fees.spotTaker;
  const exitFee        = perpExitFee + spotExitFee;
  const totalRoundTrip = entryFee + exitFee;
