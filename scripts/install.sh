...
for dir in keeper vault-setup; do
  ...
  if [ ! -f "$ENV_FILE" ] && [ -f "$EXAMPLE" ]; then
    ...
    warn "→ Open $dir/.env and fill in your values before deploying"
  elif [ -f "$ENV_FILE" ]; then
    ok "$dir/.env already exists — not overwriting"
  fi
done
...
echo -e "  Next steps:"
echo ""
echo -e "  ${CYAN}1.${NC} Read  docs/SETUP.md  for the full deployment guide"
