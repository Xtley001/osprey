...
// This package runs under two different runtimes: Vite (apps/web, exposes
// `import.meta.env.VITE_*`) and plain Node (packages/server, no import.meta.env
// at all — reading it directly throws). readEnv() checks both, so the same
// VITE_-prefixed variable names work as env vars for either consumer —
// one naming convention across the whole system rather than two.
...
export const ENABLE_TESTNET = readEnv('VITE_ENABLE_TESTNET') === 'true';

export const HL_REST_URL = ENABLE_TESTNET
  ? (readEnv('VITE_HL_TESTNET_REST') ?? 'https://api.hyperliquid-testnet.xyz')
  : (readEnv('VITE_HL_REST_URL')     ?? 'https://api.hyperliquid.xyz');

export const HL_WS_URL = ENABLE_TESTNET
  ? (readEnv('VITE_HL_TESTNET_WS') ?? 'wss://api.hyperliquid-testnet.xyz/ws')
  : (readEnv('VITE_HL_WS_URL')     ?? 'wss://api.hyperliquid.xyz/ws');

// OP-09 hardened: real trading requires explicit opt-in (=== 'true'), not opt-out.
export const ENABLE_REAL_TRADING  = readEnv('VITE_ENABLE_REAL_TRADING')  === 'true';
export const ENABLE_AGENT_KEYS    = readEnv('VITE_ENABLE_AGENT_KEYS')    !== 'false';
export const WALLETCONNECT_PROJECT = readEnv('VITE_WALLETCONNECT_PROJECT_ID') ?? '';
