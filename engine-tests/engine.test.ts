...
describe('runHarvestCycle — regime gate', () => {
  it('produces no ENTER actions in COLD regime when regimeGate is on', () => {
    ...
    const { actions } = runHarvestCycle(pairs, [], COLD_REGIME, DEFAULT_CONFIG);
  ...
  it('logs a regime-paused INFO message in COLD regime', () => {
    const pairs = [makePair({ currentRate: 0.001, openInterest: 5_000_000 })];
    const { logLines } = runHarvestCycle(pairs, [], COLD_REGIME, DEFAULT_CONFIG);
  ...
  it('allows ENTER in COLD regime when regimeGate is disabled', () => {
    ...
    const { logLines } = runHarvestCycle(pairs, [], COLD_REGIME, config);
...
describe('runHarvestCycle — exits', () => {
  it('emits EXIT when position rate drops below exit threshold', () => {
    ...
    const { actions } = runHarvestCycle(pairs, [pos], HOT_REGIME, DEFAULT_CONFIG);
...
describe('runHarvestCycle — entries', () => {
  it('does not enter when max positions is already reached', () => {
    ...
    const { actions } = runHarvestCycle(pairs, positions, HOT_REGIME, config);
    const enters = actions.filter(a => a.type === 'ENTER');
    expect(enters.length).toBe(0);
  });
  ...
  it('skips pairs with OI below minOI', () => {
    ...
    const { actions } = runHarvestCycle(pairs, [], HOT_REGIME, DEFAULT_CONFIG);
    const enters = actions.filter(a => a.type === 'ENTER');
    expect(enters.length).toBe(0);
  });
...
describe('runHarvestCycle — log entries', () => {
  it('every action produces at least one corresponding log line', () => {
    ...
    const { logLines } = runHarvestCycle(pairs, [], HOT_REGIME, DEFAULT_CONFIG);
