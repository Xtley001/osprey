...
// @osprey/engine is a single barrel package (engine logic + HL API + fees).
// Only the HL API surface is mocked here — runHarvestCycle/detectRegime/etc.
// come through unmocked via importOriginal so the real engine logic still runs.
...
describe('harvestStore smoke tests', () => {
  ...
  it('runCycle disabled = no-op', async () => {
    const { placeMarketOrder } = await import('@osprey/engine');
  ...
  it('HL_REST_URL/HL_WS_URL are non-empty strings', async () => {
    const { HL_REST_URL, HL_WS_URL } = await import('@osprey/engine');
