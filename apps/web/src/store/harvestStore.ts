    ...
    try {
      ...
      for (const action of actions) {
        ...
        if (action.type === 'ENTER') {
          ...
          const { buildPortfolio, DEFAULT_PORTFOLIO_CONFIG } = await import('@osprey/engine');
        ...
        if (action.type === 'ROTATE') {
          ...
          const { buildPortfolio: bpRotate, DEFAULT_PORTFOLIO_CONFIG: dpcRotate } = await import('@osprey/engine');
      ...
      if (config.hedgeMode === 'hl_spot' && wallet.connected && wallet.address) {
        ...
        if (getSignerFn || hasInjected) {
          ...
          for (const pos of currentPositionsForRebalance) {
            ...
            const { computeRebalanceDelta } = await import('@osprey/engine');
