 ...
 * 01-create-vault.ts
 *
 * Creates the Ranger vault on Solana.
 * Run ONCE. Save the output VAULT_ADDRESS to your .env.
 *
 * Prerequisites:
 *   - RPC_URL, ADMIN_KEYPAIR, MANAGER_KEYPAIR, VAULT_KEYPAIR, USDC_MINT in .env
 *   - Admin wallet funded with ~0.1 SOL for rent
 */
...
async function main() {
  ...
  const ix = await client.createInitializeVaultIx(
    {
      config: {
        maxCap:                          new BN(10_000_000_000_000), // 10M USDC (6 decimals)
        startAtTs:                       new BN(Math.floor(Date.now() / 1000)),
        lockedProfitDegradationDuration: new BN(21600), // 6h unlock window
        managerManagementFee:            100,            // 1% annualised
        managerPerformanceFee:           2000,           // 20% performance fee
        adminManagementFee:              0,              // Ranger protocol fee — set per their terms
        adminPerformanceFee:             0,
        redemptionFee:                   0,
        issuanceFee:                     0,
        withdrawalWaitingPeriod:         new BN(0),      // set to 259200 (3 days) for production
      },
